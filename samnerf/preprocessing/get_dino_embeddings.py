...
if __name__ == "__main__":
    ...
    if os.path.isdir(args.image_path):
        ...
        with torch.no_grad():
            ...
            for imgpath in glob.glob(os.path.join(args.image_path, "*")):
                if imgpath.endswith(".jpg") or imgpath.endswith(".png"):
                    image_batch, image_pil = extractor.preprocess(imgpath, args.load_size)
                    print(f"Image {imgpath} is preprocessed to tensor of size {image_batch.shape}.")
                    ...
                    savepath = os.path.join(args.output_path, os.path.basename(imgpath).split(".")[0] + ".pt")
                    torch.save(descriptors, savepath)
                    print(f"Descriptors saved to: {savepath}")
    else:
        with torch.no_grad():
            device = "cuda" if torch.cuda.is_available() else "cpu"
            extractor = ViTExtractor(args.model_type, args.stride, device=device)
