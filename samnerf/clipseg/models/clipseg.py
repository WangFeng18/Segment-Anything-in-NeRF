        ...
        with torch.no_grad():
            ...
            if self.n_tokens is not None:
                ...
                conv_weight2 = nnf.interpolate(
                    self.model.conv1.weight, (stride2, stride2), mode="bilinear", align_corners=True
                )
                x = nnf.conv2d(
                    x_inp, conv_weight2, bias=self.model.conv1.bias, stride=stride2, dilation=self.model.conv1.dilation
                )
            else:
...
class CLIPDensePredT(CLIPDenseBase):
    def __init__(
        ...
        if fix_shift:
            ...
            self.shift_vector = nn.Parameter(
                torch.load(join(dirname(basename(__file__)), "shift_text_to_vis.pth")), requires_grad=False
            )
    ...
    def forward(
        ...
        mask=None,
        return_clip_feature=False,
        debug=False,
    ):
        assert type(return_features) == bool

        # x_inp = normalize(inp_image)
        if inp_feature is None:
            x_inp = inp_image
            ...
            if debug:
                for _i in range(len(_activations)):
                    _activations[_i] = torch.cat(
                        [_activations[_i][1:].mean(dim=0, keepdim=True), _activations[_i][1:]], dim=0
                    )
            _ret_activations = []
            a = None
            for i, (activation, block, reduce) in enumerate(zip(_activations, self.blocks, self.reduces)):
                reduced_activation = reduce(activation)
                _ret_activations.append(reduced_activation)
                if a is not None:
                    a = reduced_activation + a
                else:
                    a = reduced_activation
            ...
            if return_clip_feature:
                return {
                    "visual_q": visual_q,
                    "activations": _ret_activations,
                    "transformed_image_size": transformed_image_size,
                }
        else:
            visual_q = inp_feature["visual_q"]
            reduced_activations = inp_feature["activations"]
            # bs = reduced_activations[0].shape[1]
            bs = 1
        ...
        a = self.trans_conv(a)
        if self.n_tokens is not None:
            a = nnf.interpolate(a, transformed_image_size, mode="bilinear", align_corners=True)
...
class CLIPDenseBaseline(CLIPDenseBase):
    def __init__(
        ...
        self.reduce2 = nn.Sequential(nn.Linear(reduce_dim, reduce2_dim), nn.ReLU(), nn.Linear(reduce2_dim, reduce_dim))
